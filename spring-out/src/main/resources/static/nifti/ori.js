...
window.onload = function () {
    ...
    downloadAndView1(niftiName);
};
const urlParams1 = new URLSearchParams(window.location.search);
let dicUrl1 = urlParams1.get('imageId');
function loadr1(){
    window.location.href = `http://localhost:8081/viewOriginal?imageId=${dicUrl1}`
}
function loadr2(){
...
function downloadAndView(niftiName) {
    let url = `/Files/Original/${niftiName}`;
...
function loadAndViewImage(imageId) {
    //获取图像信息
    const element = document.querySelector('.cornerstone-element1');
...
//从前端获取文件名字在这里执行
function downloadAndView1(niftiName) {
    let url = `/Files/Nifti/${niftiName}`;
    loadAndViewImage1(`nifti:${url}`);
}

function loadAndViewImage1(imageId) {
    //获取图像信息
    const element = document.querySelector('.cornerstone-element2');
    cornerstone.enable(element);
    const imageIdObject = ImageId.fromURL(imageId);
